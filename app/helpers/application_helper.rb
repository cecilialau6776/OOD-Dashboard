...
module ApplicationHelper
  ...
  def current_app_url?(sidebar_url)
    return false unless sidebar_url.present?

    # First check if the sidebar URL is an app show route
    if (match = sidebar_url.to_s.match(%r{/apps/show/([^/]+)(?:/([^/]+))?(?:/([^/]+))?}))
      name, type, owner = match.captures
      
      # Create an app object using the same logic as AppsController#show
      app = ::OodApp.new(router_for_type(type || 'sys', owner, name))
      app_url = app.url.to_s
      
      # Compare with the current path
      request.path == app_url
    else
      # Direct comparison for non-app URLs
      request.path == sidebar_url.to_s
    end
  rescue => e
    false
  end
