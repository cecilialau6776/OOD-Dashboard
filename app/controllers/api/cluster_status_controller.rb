  ...
  class ClusterStatusController < ApplicationController
    def get
      nodes = Rails.cache.fetch('cluster_status', expires_in: 30.seconds, race_condition_ttl: 3.seconds) do
        get_cluster_nodes('sporc') + get_cluster_nodes('tigris')
      end
    ...
    def get_cluster_nodes(cluster)
      output, status = Open3.capture2("scontrol show node -a --oneliner --cluster=#{cluster}")

      raise "Failed to fetch cluster '#{cluster}' status" unless status.success?
