  ...
  class JobsController < ApplicationController
    ...
    COMPUTED_FIELDS = {
      ...
      'time_efficiency' => {
        type: :float,
        compute: lambda { |data|
          data['Elapsed'] / data['Timelimit'].to_f if data['Elapsed'] && data['Timelimit']
        }
      },
      'UsedGPUHours' => {
        type: :float,
        compute: lambda { |data|
          if data['Partition'] && data['QOS'] && data['Elapsed'] && data['Timelimit'] && data['ReqTRES']
            used_gpu_hours, = Util.get_gpu_hours_usage(
              data['JobID'],
              data['Partition'],
              data['QOS'],
              data['Elapsed'],
                ...
                if session.view
                  ...
                  if views.size == 1
                    ...
                    views.each_with_index do |view, idx|
                      ...
                      tabs_html += "<a href='#c_#{session.id}_#{idx}' data-toggle='tab' aria-selected='#{idx.zero?}' class='nav-link #{if idx.zero?
                                                                                                                                         'active'
                                                                                                                                       end}'>#{view[:title]}</a>"
                      tabs_html += '</li>'
                    end
                    ...
                    views.each_with_index do |view, idx|
                      content_html += "<div id='c_#{session.id}_#{idx}' class='tab-pane ood-appkit markdown #{if idx.zero?
                                                                                                                'active'
                                                                                                              end}' role='tabpanel'>"
      ...
      'SessionConnectionInfo' => {
        ...
        compute: lambda { |data|
          if data['JobID']
            ...
            session.render_info_view if session&.app&.session_info_view
          end
        }
      }
    }.freeze
    ...
    def show
      ...
      job_data = Rails.cache.fetch("job_data/#{cluster}/#{job_id}", expires_in: 1.second) do
        get_job_info(cluster, job_id)
      end

      return head :not_found if job_data.nil?
    ...
    def cancel
      ...
      return render json: { error: 'Invalid job ID format.' }, status: :bad_request unless /^\d+(_\d+)?$/.match?(job_id)

      # Get job info to check ownership
      job_data = get_job_info(cluster, job_id)

      return render json: { error: "Job #{job_id} not found." }, status: :not_found if job_data.nil?
    ...
    def get_job_info(cluster, job_id)
      # Validate job ID format
      return nil unless /^\d+(_\d+)?$/.match?(job_id)
      ...
      scontrol_output, scontrol_status = Open3.capture2e("scontrol -M #{cluster} show job #{job_id} -d -o")
      sacct_output, sacct_status = Open3.capture2e("sacct -M #{cluster} -j #{base_job_id} -P -n --array --expand-patterns --format=#{sacct_fields}")
      ...
      unless sacct_output.blank?
        ...
        sacct_output.split("\n").each do |line|
          ...
          sacct_fields.split(',').each_with_index do |field, index|
            ...
            value = convert_cancelled_state(value) if field == 'State'
        ...
        FIELD_INFO.each do |_, info|
          next unless info[:combinator] == :set_union && info[:sacct]
    ...
    def convert_cancelled_state(value)
      if value =~ /^CANCELLED by (\d+)$/
        username = Rails.cache.fetch("user_id/#{::Regexp.last_match(1)}") do
          `id -un #{::Regexp.last_match(1)}`.strip
        end
    ...
    def merge_job_data(scontrol_data, sacct_data, sacct_output, sacct_fields)
      ...
      merged_data['JobArray'] = if job_id && has_array_tasks?(sacct_output, sacct_fields)
                                  collect_job_array_data(sacct_output, sacct_fields, job_id)
                                else
                                  []
                                end

      # Change PENDING to REQUEUED if there are restarts
      merged_data['State'] = 'REQUEUED' if merged_data['Restarts'] && (merged_data['Restarts']&.to_i&.> 0)
    ...
    def timestr_to_seconds(timestr)
      ...
      if d_hhmmss_regex.match?(timestr)
        ...
        total_seconds = days * 86_400 + hours * 3600 + minutes * 60 + seconds
