...
class MyJobsController < ApplicationController
  ...
  FIELDS = {
    'cluster' => { 'prefer' => 'first', 'format' => 'string', 'formattedname' => 'Cluster' },
    ...
    'start' => { 'prefer' => 'first', 'format' => 'timestamp', 'formattedname' => 'Start Time',
                 'compare_fn' => Util.method(:timestamp_to_epoch) },
    'end' => { 'prefer' => 'first', 'format' => 'timestamp', 'formattedname' => 'End Time',
               'compare_fn' => Util.method(:timestamp_to_epoch) },
    'elapsed' => { 'prefer' => 'max', 'format' => 'duration', 'formattedname' => 'Elapsed',
                   'compare_fn' => Util.method(:timestr_to_seconds) },
    'submit' => { 'prefer' => 'first', 'format' => 'timestamp', 'formattedname' => 'Submit Time',
                  'compare_fn' => Util.method(:timestamp_to_epoch) },
    'planned' => { 'prefer' => 'max', 'format' => 'duration', 'formattedname' => 'Planned Time',
                   'compare_fn' => Util.method(:timestr_to_seconds) },
    ...
    'timelimit' => { 'prefer' => 'max', 'format' => 'duration', 'formattedname' => 'Timelimit',
                     'compare_fn' => Util.method(:timestr_to_seconds) },
    'reqmem' => { 'prefer' => 'first', 'format' => 'size', 'formattedname' => 'Requested Memory',
                  'compare_fn' => Util.method(:to_bytes) },
    'alloccpus' => { 'prefer' => 'first', 'format' => 'number', 'formattedname' => 'Allocated CPUs' },
    'totalcpu' => { 'prefer' => 'max', 'format' => 'duration', 'formattedname' => 'Total CPUs',
                    'compare_fn' => Util.method(:timestr_to_seconds) },
    ...
    'maxrss' => { 'prefer' => 'max', 'format' => 'size', 'formattedname' => 'Max Memory Used',
                  'compare_fn' => Util.method(:to_bytes) },
    'maxdiskwrite' => { 'prefer' => 'max', 'format' => 'size', 'formattedname' => 'Max Disk Write',
                        'compare_fn' => Util.method(:to_bytes) },
    'maxdiskread' => { 'prefer' => 'max', 'format' => 'size', 'formattedname' => 'Max Disk Read',
                       'compare_fn' => Util.method(:to_bytes) },
  ...
  SACCT_FIELDS = FIELDS.reject { |_, value| value['prefer'] == 'created' }.keys
  ...
  def cancel_jobs
    ...
    return head :bad_request if job_ids.empty?

    job_ids.each do |job_id|
      return head :bad_request unless /^\d+(_\d+)?$/.match?(job_id)
    end

    # All scancel output is in stderr instead of stdout for some reason
    result, = Open3.capture2e("scancel -M #{cluster} -u #{@user.name} -v #{params[:job_ids]}")

    cancelled_job_ids = result.split("\n").map do |line|
      ::Regexp.last_match(1) if line =~ /^scancel: Terminating job (\d+(?:_\d+)?)$/
    end.compact
  ...
  def json
    ...
    if params[:start_time].present?
      ...
      return render json: { error: 'Invalid start time.' }, status: :bad_request if start_time.nil?
    end

    if params[:end_time].present?
      end_time = Time.zone.parse(params[:end_time])
      return render json: { error: 'Invalid end time.' }, status: :bad_request if end_time.nil?
    end

    return render json: { error: 'Start time cannot be after end time.' }, status: :bad_request if start_time > end_time

    allocations = Util.get_user_allocations(@user.name)

    squeue = Rails.cache.fetch('squeue', expires_in: 1.seconds) do
      output, status = Open3.capture2('squeue -t all -h -o "%i|%P|%j|%u|%T|%r"')
    ...
    result, status = Open3.capture2("sacct -L -S #{start_time.strftime('%FT%T')} -E #{end_time.strftime('%FT%T')} -P -n -a -o #{SACCT_FIELDS.join(',')} -A #{allocations}")
    ...
    result.each_line do |line|
      ...
      if !jobs.key?(job_key)
        jobs[job_key] = FIELDS.keys.map { |field| [field, ''] }.to_h.merge(SACCT_FIELDS.zip(s).to_h)
        ...
        PREFER_MAX_FIELDS.each do |field|
          jobs[job_key][field] = [jobs[job_key][field], s[SACCT_FIELDS.index(field)]].max_by do |val|
            FIELDS[field]['compare_fn'].call(val)
          end
        end
      end
    end
    ...
    jobs_hash = jobs.values.map do |j|
      ...
      if j['state'] == 'REQUEUED'
        output, status = Open3.capture2("scontrol show job #{j['jobid']} -o")
        if status.success?
