...
function promiseLoginToXDMoD(xdmodUrl) {
  return new Promise(function(resolve, reject) {
    ...
    fetch(xdmodUrl + "/rest/auth/idpredirect?returnTo=%2Fgui%2Fgeneral%2Flogin.php").then((response) => response.ok ? Promise.resolve(response) : Promise.reject()).then((response) => response.json()).then(function(data) {
...
var promiseLoggedIntoXDMoD = function() {
  return import_lodash.default.memoize(function(xdmodUrl) {
    return fetch(xdmodUrl + "/rest/v1/users/current", { credentials: "include" }).then((response) => {
      if (response.ok) {
        return Promise.resolve(response.json());
      } else {
        return promiseLoginToXDMoD(xdmodUrl).then(() => fetch(xdmodUrl + "/rest/v1/users/current", { credentials: "include" })).then((response2) => response2.json());
      }
    }).then((user_data) => {
...
window.Handlebars = import_handlebars.default;
jQuery(function() {
