...
Rails.application.routes.draw do
  if Configuration.can_access_projects?
    ...
    resources :projects do
      root 'projects#index'
      get '/jobs/:cluster/:jobid' => 'projects#job_details', :defaults => { format: 'turbo_stream' },
          :as => 'job_details'
  ...
  if Configuration.can_access_files?
    ...
    resources :transfers, only: %i[show create destroy]
  end
  ...
  namespace :batch_connect do
    ...
    scope '*token', constraints: { token: %r{((usr/[^/]+)|dev|sys)/[^/]+(/[^/]+)?} } do
      ...
      resources :session_contexts, only: %i[new create]
  ...
  scope 'admin/:type', constraints: Authz::AppDeveloperConstraint do
    resources :products, except: :destroy, param: :name, constraints: { type: /dev|usr/ } do
      nested do
        scope ':context' do
          resources :permissions, only: %i[index new create destroy], param: :name
        end
      end
      member do
  ...
  if Configuration.widget_partials_enabled?
    match '/widgets/*widget_path', to: 'widgets#show', via: %i[get post], as: 'widgets'
  end
  ...
  delete '/:cluster/myjobs' => 'my_jobs#cancel_jobs', as: 'cancel_jobs'

  get '/job/:cluster/:jobid/json' => 'job_info#json'
