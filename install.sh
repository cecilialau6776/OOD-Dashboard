...
# Check if the script is running from the OOD-Dashboard repository
REPO_URL="https://github.com/PurdueRCAC/OOD-Dashboard"
USE_CURRENT_DIR=false

if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  REMOTE_URL=$(git config --get remote.origin.url)
  if [[ "$REMOTE_URL" == "$REPO_URL" || "$REMOTE_URL" == "git@github.com:PurdueRCAC/OOD-Dashboard.git" ]]; then
...
if $USE_CURRENT_DIR; then
  ...
  info "Detected script is running from the OOD-Dashboard repository. Using current directory: $DASHBOARD_DIR"
else
  ...
  if [ "$use_ssh" == "yes" ]; then
    git clone git@github.com:PurdueRCAC/OOD-Dashboard.git "$DASHBOARD_DIR" >/dev/null 2>&1
  else
    git clone https://github.com/PurdueRCAC/OOD-Dashboard "$DASHBOARD_DIR" >/dev/null 2>&1
  fi
...
info "You can now access the development dashboard."
